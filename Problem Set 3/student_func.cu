...
__global__ void reduce_minimum(float * d_out, const float * d_in, const size_t numItem){
   //sdata[] is allocated for shared memory
...
__global__ void reduce_maximum(float * d_out , const float * d_in , const size_t numItem){
   extern __shared__ float sdata[];
...
__global__ void histogram(unsigned int * d_histogram, 
   ...
   unsigned int numRows, unsigned int numCols)
{
   ...
   unsigned int bin = min(static_cast<unsigned int>(numBins - 1),
                           static_cast<unsigned int>((d_logLuminance[myId] - min_logLum) / range * numBins));
   
   atomicAdd(&(d_histogram[bin]),1);
}


__global__ void scan_per_block(
   unsigned int * d_cdf,
   unsigned int * d_sums, 
   ...
   unsigned int numElems)
{
   ...
   sdata[tid * 2 + 1] = .0f;
   if( myId * 2 < numBins){
      sdata[tid * 2] = d_histogram[myId * 2];
      
   }
   if(myId *2 + 1 <numBins){
      sdata[tid * 2 + 1] = d_histogram[myId *2 +1];
   }
   __syncthreads();
   ...
   for(unsigned int s = 1 ; s <= blockDim.x ; s <<=1){
      if(tid < blockDim.x /s){
         sdata[2 * s *(tid + 1)-1] += sdata[s*(2 * tid + 1) -1];
      }
      __syncthreads();
   }
   ...
   for(unsigned int s = blockDim.x; s > 0;s>>=1){
      if(tid < blockDim.x /s){
         float tmp = sdata[s*(2 * tid + 1) -1];
         sdata[s*(2 * tid + 1) -1] = sdata[2*s*(tid+1)-1];
         sdata[2*s*(tid+1)-1] += tmp;
      }
      __syncthreads();
   }
...
__global__ void scan_across_block(
   unsigned int * d_incr,
   const unsigned int * d_sums,
   ...
   const unsigned int numElems)
{
   ...
   if ((2 * myId) < numBins) {
      d_incr[2 * myId] = sdata[2 * tid];
    }
    if ((2 * myId + 1) < numBins) {
      d_incr[2 * myId + 1] = sdata[2 * tid + 1];
    }
                                  ...
                                  const size_t numBins)
{
  ...
  /*Here are the steps you need to implement
   ...
   unsigned int numItem = numCols * numRows;
   ...
   reduce_minimum<<<gridSize, blockSize , sizeof(float) *blockSize.x>>>
   (d_inter_min, d_logLuminance, numItem);
   
   reduce_maximum<<<gridSize, blockSize, sizeof(float) * blockSize.x>>>
   (d_inter_max, d_logLuminance, numItem);
   numItem = gridSize.x;
   gridSize.x = numItem/blockSize.x +1;
   
   while(numItem > 1){
      reduce_minimum<<<gridSize, blockSize , sizeof(float) *blockSize.x>>>
      (d_inter_min, d_inter_min, numItem);
      
      reduce_maximum<<<gridSize, blockSize, sizeof(float) * blockSize.x>>>
      (d_inter_max, d_inter_max, numItem);
      numItem = gridSize.x;
      gridSize.x = numItem/blockSize.x +1;
   }
   ...
   checkCudaErrors(cudaMalloc(&d_incr, sizeof(unsigned int ) * gridSize.x));
   checkCudaErrors(cudaMemset(d_sums, 0, sizeof(unsigned int) * gridSize.x));
   checkCudaErrors(cudaMemset(d_incr, 0, sizeof(unsigned int) * gridSize.x));
   int numElems = 256;
   ...
   scan_per_block<<<gridSize, blockSize, sizeof(float) * numElems>>>(d_cdf, d_sums, d_histogram, numBins, numElems);
   numElems = gridSize.x;
   gridSize.x =1 ;
   // 变成2的倍数
   ...
   scan_across_block<<<gridSize, blockSize,sizeof(float) * nextPow>>>(d_incr, d_sums, numElems, nextPow);
